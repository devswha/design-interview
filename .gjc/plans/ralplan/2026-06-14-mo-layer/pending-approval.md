# 최종 합의 계획 (pending approval) — MO 빌드 레이어 + 시각 크래프트 통합

run-id: 2026-06-14-mo-layer / stage: final / 상태: **pending approval** (승인 전 product 소스·audit·SKILL 미변경, 실행/위임 없음)

## 합의 기록 (consensus record)

| 패스 | Planner | Architect | Critic |
|---|---|---|---|
| 1 | stage-01-planner (옵션 B 권고) | WATCH / COMMENT (1 HIGH + 3 MED + WATCH, 코드 대조) | ITERATE (9 액션) |
| 2 | stage-02-revision (9항 + 정정 반영) | **CLEAR / APPROVE** (전항 해소·신규 결함 없음) | **OKAY** (실행 핸드오프 가능) |

2 iteration 만에 합의. 모든 결함은 src/audit.js 코드 대조로 검증됨.

## ADR

- **Decision**: Architect Synthesis 3안 채택. (1) **M8.1 문서·프롬프트 + M8.3 시각 임팩트는 본 계획 승인 즉시 실행 가능**(코드 무변경, 실험으로 0% 입증). (2) **M8.2 기계검사는 게이트형 분리 슬라이스** — 첫 산출물이 `at-rule/@keyframes 중첩 보존 파서 헬퍼`(extractCssRules 불변)이고, 그 헬퍼가 적대 심사를 통과한 뒤에만 b1을 **WARN**(collectWarnings 단독·MACHINE_CHECKS 미등록·baseline 무변경)으로 승격한다. b3 hover-only는 decoy 우회를 닫기 전까지 LLM 잔류. (3) **M7 reference intelligence는 후순위 디스코프**(M7.1 별도 run, M7.2/3 보류).
- **Drivers**: D1 인터랙티비티를 보안/감사 약화 없이 도입(=CSS-first 무JS). D2 모션이 anti-slop 테제를 훼손하지 않게(MO1 장식모션 텔 + cap-not-quota). D3 기계 승격은 적대 심사 **+ 파서 구비**가 충족될 때만.
- **Alternatives considered**: 옵션 A(문서만) — 리스크 최저지만 모션 회귀 방지 약함 → A 즉효분은 채택, 결정론 보강을 게이트로 후행. 옵션 B(기계검사 즉시) — extractCssRules가 @media 미보존이라 b1 즉시 구현 불가·"exp WARN0" 자기모순 → 파서 선행 게이트로 재정의해 흡수. 옵션 C(+레퍼런스 병행) — 실험상 레퍼런스 비병목·P1 위반·스코프 분산 → 기각, M7.1 별도 run 보존.
- **Why chosen**: 실험이 빌드 레이어가 병목임을 데이터로 입증(exp 0%/인터랙티브 vs baseline 7%/정적). 즉효(A)를 막지 않으면서, 결정론(B)은 파서 비용을 "저비용 WARN" 뒤에 숨기지 않고 게이트로 노출. cap-not-quota·이중채점 금지·inert-safe·claim 불변 전부 보존.
- **Consequences**: (+) 산출물이 무JS로 인터랙티브·시각적으로 강해짐. (+) 모션 규율 일부가 결정론 WARN으로 회귀 방지. (−) 중첩 파서 신설은 회귀면적이 있음(extractCssRules 불변 + 단위테스트로 제한). (−) WARN 승격해도 빈 가드·decoy 우회는 잔존(의도된 한계, FAIL 비승격 근거). (−) 레퍼런스 인텔리전스는 미뤄짐.
- **Residual LOW (Architect2, 본 final 반영)**: FAIL 경로를 택할 경우 checkQualityFloor의 5번째 arm은 평탄 `extractCssRules` 결과가 아니라 **M8.2에서 신설하는 중첩 보존 헬퍼**를 소비해야 한다(평탄 rules로는 가드 컨텍스트 판별 불가 — exp 거짓 FAIL 위험). M8.2 구현 시 이 한 줄을 검사 명세에 못박을 것.
- **Follow-ups (승인자 입력 필요한 미해결 결정)**: (1) MO 텔 id 컨벤션 — `C6 animated-decoration` 확장 vs 신규 `M*` 모션군. (2) 파서 헬퍼가 적대 심사 통과 시 b1을 WARN 확정할지, 보수적으로 옵션 A(전부 LLM)로 잔류할지. (3) M7.1 레퍼런스 큐레이션을 별도 run으로 분리하는 데 동의하는지.

---

## 승인된 계획 본문 (rev2 — Architect CLEAR/APPROVE · Critic OKAY)
