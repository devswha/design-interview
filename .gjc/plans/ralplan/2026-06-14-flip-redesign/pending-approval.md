# 최종 합의 계획 (pending approval) — 뒤집기 재설계: subtractive → additive

run-id: 2026-06-14-flip-redesign / stage: final / 상태: **pending approval** (승인 전 product·.gjc 미변경, 실행/위임 없음)

## 합의 기록

| 패스 | Planner | Architect | Critic |
|---|---|---|---|
| 1 | stage-01 (Option B) | WATCH / REQUEST CHANGES (분할선·게이트 2 HIGH) | ITERATE (P0×2+P1/P2) |
| 2 | stage-02-revision | **CLEAR / COMMENT** (8 해소·차단 0) | **OKAY** (코드 검증·핸드오프 가능) |

코드 대조로 검증됨: slop-source.failed=[C1,T1,T2,T4] 전부 blocking→exit 1 보존, benchmark union 비교 무변경, slopScore 키 존속, cli exit 불변.

## ADR

- **Decision**: design-interview의 철학을 **빼기(audit-주도·절제) → 더하기(에셋/레퍼런스 라이브러리·디자인 우선)**로 뒤집는다. audit은 게이트가 아니라 **2채널 가드레일**:
  - **차단(blocking)** = 품질 바닥선(DE3 4암·렌더 대비·reduced-motion·접근성) **+ 하드 슬롭 지문(C1 보라그라데·T1 이모지·T2 hype·T4 대칭3연)**.
  - **권고(advisory)** = *표현 억제* 휴리스틱(TY4 패밀리 상한·CO1 색 예산·DE1 그림자 캡·S5 radius·시각 L1/L2/S3·TY1/TY2) — 의도로 초과 가능, 납품 차단 안 함.
  - **게이트 2개 분리**: CLI exit = blockingFailed만(납품) / benchmark = blocking+advisory 전체 miss+fp(탐지기 정확도 — 회귀망 보존).
  - **에셋/레퍼런스 라이브러리 1급 입력**(assets/{fonts,textures,icons,palettes}, refs/{screenshots,brief}) — 빌드가 *조립*. 자가호스팅 폰트·에셋 허용(webfont 금지 철학 폐기).
- **Drivers**: ① 탐지기(patina)+엔터프라이즈 디자인시스템 기반 audit은 '슬롭 없음=무난한 디폴트'가 천장 → 빼기로는 특색 불가. ② v6 실증: 더 나은 디자인을 옛 audit이 차단(TY4 3패밀리·L1). ③ 사용자 기준 "더 살아있게" → 에셋·인터랙션이 1급이어야.
- **Alternatives**: (A) 문서/프롬프트만 분할 — 결정론 약함 → 흡수. (B 채택) audit 2채널 + 에셋 라이브러리. (C) 시각레인 재설계 — 후속 run 디스코프.
- **Why chosen**: 표현은 풀되(advisory), *슬롭 재발은 결정론적으로 막는다* — 하드텔 blocking + 품질 바닥선이 진짜 안전망(POV·큐레이션은 LLM 비결정론 보조). 회귀망(benchmark)은 union으로 보존. 클레임 보존·inert 보안 불변.
- **Consequences**: (+) 산출물이 에셋·인터랙션으로 살아남, 좋은 디자인을 audit이 더는 차단 안 함. (+) 슬롭 지문·품질 바닥선은 여전히 결정론적 차단. (−) advisory 강등분(L1 등)은 신호 약화 — benchmark 회귀망으로 상쇄. (−) 에셋 라이브러리·2채널은 src/audit.js·cli.js·baseline·core 문서 다중 변경.
- **Follow-ups / 비차단 핸드오프**:
  1. (MEDIUM) e2e 2/4b의 `FAIL S3` 단언이 S3 advisory 강등과 충돌 — 슬라이스 1/2에서 처리(advisory도 per-finding FAIL 라벨 유지 + BLOCK/advise 섹션 구분, 또는 2/4b 단언을 advise로 갱신). npm-test-green 게이트가 강제 해소.
  2. (LOW) 합성 finding severity 부여 시 geometry.test.js `combineAudits pass` 단언 동기화(severity 누락 시 blocking fail-safe 기본).
  3. C(시각레인 재설계)·M7.1(레퍼런스 큐레이션 문서)은 별도 run.

---

## 승인된 계획 본문 (rev2 — Architect CLEAR · Critic OKAY)
