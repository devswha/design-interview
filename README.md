...
옵션 보드 CLI (Phase 1 — 시각 인터뷰):

```bash
node src/cli.js board round2.options.json --out board.html
# → file:///…/board.html  (브라우저로 열어 옵션 카드 비교, 추천 배지 표시, 직접 입력 카드 포함)
# options.json: schemaVersion/boardId/roundId + dimension(6 core) + options[]
...
| `src/preview.js` | inert 프리뷰 빌더 — CSP + 무스크립트 radio 토글 (inert-html.js 공유) |
| `src/board.js` | Phase 1 옵션 보드 직렬화 — 검증된 옵션 모델 → inert 단일 HTML, magic-byte 검증 이미지 임베드, atomic write |
...
- **Visual interview**: 인터뷰 선택지는 텍스트가 아니라 브라우저 옵션 보드로 보여준다 — 사용자가 방향을 눈으로 비교하고 고른다. 보드는 판단하지 않고 검증된 옵션 모델을 inert HTML로 직렬화만 한다(추천·점수·열기는 호스트 레이어).
