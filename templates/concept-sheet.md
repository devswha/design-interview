...
| 차원 | 결정 | 추천→선택 | 최종 점수 |
|---|---|---|---|
| audience | {도착 맥락 한 문장} | {추천 ○○ → 선택 ○○ · 일치면 "추천 채택"} | {0.00} |
| mood | {형용사 + 대비 진술} | {추천 → 선택} | {0.00} |
| brand | {자산 유무, 색/서체} | {추천 → 선택} | {0.00} |
| structure | {첫 화면 우선순위} | {추천 → 선택} | {0.00} |
| conversion | {단일 전환 행동 + CTA 방향} | {추천 → 선택} | {0.00} |
| reference | {URL/DESIGN.md + 빌릴 레이어·버릴 레이어·복제 금지 조건} | {추천 → 선택} | {0.00} |
