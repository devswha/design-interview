...
1. **기계 감사 (결정론적)**: `node src/cli.js audit <built.html> --visual` 실행. 정적 레인은 텔(C1/T1/T2/T4/S5)과 원칙(TY4 패밀리 규율, CO1 색 리터럴 예산, DE1 그림자 물리, DE3 품질 바닥선)을 CSS/HTML 파싱으로, 시각 레인은 텔(L1 균일 그리드, L2 전부 중앙, S3 완전 대칭)과 원칙(TY1 타입 스케일, TY2 행길이·본문 크기)을 렌더된 박스 기하로 판정한다 — exit 1이면 납품 불가, Phase 3으로 돌아가 수정 후 재실행. WARN 라인(직선 따옴표 등)은 납품을 막지 않지만 가능하면 수정한다. LLM 자기 채점으로 이 단계를 대체하지 않는다. puppeteer 미설치면 시각 레인은 자동 생략되고 정적 검사만 판정된다.
