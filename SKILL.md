...
- **클레임 동결**: `node src/cli.js intake <source> --json`으로 보존 클레임(가격·수량·백분율·기간·기능)을 구조화 추출해 잡아둔다. **클레임은 변경 불가 자산이다** — 디자인이 바뀌어도 숫자·사실·인과는 보존한다 (patina의 MPS 원칙 준수). 이 결과가 Phase 2 컨셉 시트의 "보존 클레임" 섹션과 Phase 5 대조표의 기준이 된다.
