...
- **에셋 계획(Sourcing Plan)**: 컨셉 시트 토큰 커밋 표의 **에셋 계획(Sourcing Plan)** 행을 채운다 — per-asset로 소싱 경로(보유 폴더 경로 / 생성 / 번들 샘플 `assets/samples/` / consent 크롤)와 파일·source·license를 명시한다. 인터뷰 '에셋 must-answer 규율'의 산출이며, 0개여도 어떤 경로로 구할지는 반드시 기록한다 (`core/asset-library.md` 소싱 4경로 참조). 빈 placeholder로 두면 `node src/cli.js assets <dir> --concept-sheet <sheet>`가 advisory 경고를 낸다.
