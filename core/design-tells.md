...
- **기계 레인 (정적)** (`node src/cli.js audit <built.html>`): C1, T1, T2, T4, S5는 `src/audit.js`가 HTML/CSS 파싱으로 판정한다. 원칙 검사 TY4·CO1·DE1·DE3도 이 레인.
- **기계 레인 (시각)** (`--visual` 플래그, requires puppeteer): L1, **L2**, S3는 `src/geometry.js`가 렌더된 박스 좌표·계산 스타일로 판정한다. 원칙 검사 TY1·TY2도 이 레인.
